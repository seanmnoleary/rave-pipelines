    ...
    input_subject_code = targets::tar_target_raw("subject_code", 
        ...
        command = quote({
            tryCatch({
                ...
                asNamespace("raveio")$resolve_pipeline_error("imported_electrodes", 
                  e, quote({
                    imported_electrodes <- subject$electrodes[subject$preprocess_settings$data_imported]
                    if (!length(imported_electrodes)) {
                      stop("The subject exists but its signal has not been imported yet.")
        ...
        iteration = "list"), check_filter_settings = targets::tar_target_raw(name = "filter_settings", 
        command = quote({
            tryCatch({
                {
                ...
                asNamespace("raveio")$resolve_pipeline_error("filter_settings", 
                  e, quote({
                    lb <- unlist(notch_filter_lowerbound)
                    ub <- unlist(notch_filter_upperbound)
                    if (length(lb) != length(ub)) {
                    ...
                    filter_settings <- list(lb = lb, ub = ub, 
                      domain = 1)
                  }))
            })
        }), format = asNamespace("raveio")$target_format_dynamic(NULL), 
        ...
        command = quote({
            tryCatch({
                ...
                asNamespace("raveio")$resolve_pipeline_error("apply_notch", 
                  e, quote({
                    blocks <- subject$preprocess_settings$blocks
                    electrodes <- imported_electrodes
                    filters <- filter_settings
                    fmt <- file.path(subject$preprocess_path, 
                      "voltage", "electrode_%d.h5")
                    sample_rates <- subject$raw_sample_rates
                    sample_rates <- sapply(electrodes, function(e) {
                      sample_rates[subject$electrodes == e]
                    })
        ...
        command = quote({
            tryCatch({
                ...
                asNamespace("raveio")$resolve_pipeline_error("diagnostic_plots", 
                  e, quote({
                    diagnostic_plots <- FALSE
                    params <- as.list(diagnostic_plot_params)
                    background <- params$background
