...
knitr::cache_engines$delete(c("python", "R"))
...
```{rave get_subject_instance, export = "subject", format = "rave-subject", use_rave = TRUE}
subject <- raveio::as_rave_subject(
  subject_id = sprintf("demo1/%s", subject_code), 
  strict = FALSE)
```
...
For easy debug, I manually copy everything from R to Python. Notice I use `python` block with no `use_rave` specified. This block will not be incorporated into the pipeline (debug only)
...
```{python load_original_image, export = "image_original", deps = "image_path_normalized", cue = "always", use_rave = TRUE}
...
```{python normalize_brain, export = "normalized", deps = c("image_resampled", "transforms"), use_rave = TRUE}
...
```{python generate_brain_mask, export = "brain_mask", deps = c("image_resampled", "transforms"), use_rave = TRUE}
...
```{python strip_skull, export = "skull_strip", deps = c("normalized", "brain_mask"), use_rave = TRUE}
...
```{python obtain_DKT_on_native_brain, export = "DKTatlas_native", deps = c("transforms", "DKTatlas_template", "image_resampled"), use_rave = TRUE}
