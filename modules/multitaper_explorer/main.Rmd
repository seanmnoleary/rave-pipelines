...
###@ Step 3: Run multitaper and get result
...
```
  ...
  ###@ Step 4: getheatmap data from multitaper result for each spectral range
```{rave find_heatmap, language = "R", export = "heatmap_result"}
heatmap_result <- generate_heatmap(repository, multitaper_result, time_window,  freq_list, 
                                   load_electrodes, window_params) 

```
  
  #### Step 5: Visualize Heatmaps for all the generated spectral ranges
```{r}
for(i in 1:length(heatmap_result)) {
  p <- plot_heatmap(heatmap_result[[i]])
  print(p)
}
```
