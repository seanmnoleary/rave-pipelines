...
```{rave load_power, language = "R", cue = "always", export = "repository_power", format = "rave_prepare_subject_power"}
...
### Step 4: Run multitaper and get result for all conditions
...
### Step 5: get heatmap data from multitaper result for each spectral range for a specific condition
```{rave find_heatmap, language = "R", export = "heatmap_result"}
heatmap_result <- generate_power_over_time_data(multitaper_result, analysis_time_frequencies, baselined = baselined, baseline = baseline, start_time_baseline = start_time_baseline, end_time_baseline = end_time_baseline)
```
...
```{rave generate_data_for_time_frequency_plot, language = "R", export = "plot_time_frequency", cue="always"}
plot_time_frequency <- plot_time_frequency(selected_electrode, load_electrodes, repository_power)
```
