...
heatmap_result <- generate_heatmap(repository, multitaper_result, time_window, freq_list, load_electrodes,
...
for(i in 1:length(heatmap_result)) {
  p <- plot_heatmap(heatmap_result[[i]], SOZ_elec = SOZ_elec, plot_SOZ_elec = plot_SOZ_elec,
                    name_type = label, organize_top = organize_top, repository = repository)
  print(p)
}
```
